...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!deleteResult.success || !deleteResult.deletedStudentChoices) {
      console.error(`[API Delete Submission] Failed to delete from sheet: ${deleteResult.error}`);
      return NextResponse.json({ message: deleteResult.error || 'Failed to delete submission from Google Sheet.' }, { status: 500 });
    }

    console.log('[API Delete Submission] Submission deleted from sheet. Restoring faculty slots. Choices:', deleteResult.deletedStudentChoices);
    ...
    for (const [subjectName, facultyName] of Object.entries(deleteResult.deletedStudentChoices)) {
      ...
      if (subject && faculty) {
        console.log(`[API Delete Submission] Restoring slot for Subject: ${subject.name} (ID: ${subject.id}), Faculty: ${faculty.name} (ID: ${faculty.id})`);
        const restoreResult = incrementFacultySlotSync(faculty.id, subject.id);
        if (!restoreResult.success) {
          const errorMsg = `Failed to restore slot for ${faculty.name} - ${subject.name}: ${restoreResult.error}`;
          console.error(`[API Delete Submission] ${errorMsg}`);
          slotRestorationErrors.push(errorMsg);
        } else {
          console.log(`[API Delete Submission] Slot restored for ${faculty.name} - ${subject.name}. New count: ${restoreResult.currentSlots}`);
        }
      } else {
        const errorMsg = `Could not find subject "${subjectName}" or faculty "${facultyName}" in system for slot restoration.`;
    ...
    if (slotRestorationErrors.length > 0) {
      return NextResponse.json({ 
        message: `Submission deleted for ${rollNumber}, but encountered errors restoring some faculty slots: ${slotRestorationErrors.join('; ')}`,
        partialSuccess: true 
      }, { status: 207 }); // Multi-Status
    }

    console.log(`[API Delete Submission] Successfully deleted submission and restored slots for roll number: ${rollNumber}`);
